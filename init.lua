...
vim.keymap.set("n", "<leader>n", ":tabnew ~/Dropbox/notes/")
...
else print("Telescope.nvim not found")
end
...
else print("multicursor-nvim not found")
end
...
if origami_ok then
    ...
    vim.api.nvim_create_autocmd({ "BufReadPost", "LspAttach" }, {
        callback = function()
            vim.schedule(function() vim.cmd.norm("zM") end)
        end
    })
else print("origami not found")
end
...
else print("mason or mason-lspconfig not found")
end
...
else print("typescript-tools not found")
end

local te_ok, te = pcall(require, "ts-error-translator")
if te_ok then
    te.setup()
else print("ts-error-translator not found")
end
...
else print("workspace-diagnostics not found")
end

local namu_ok, namu = pcall(require, "namu")
if namu_ok then
    namu.setup()
    vim.keymap.set("n", "grd", "<cmd>Namu diagnostics buffers<cr>", { silent = true })
